...
7. Запуск симуляции gazebo:
	...
	- cd /home/dima/ardupilot							# Переходим в папку репозитория ardupilot (новый терминал)
											# Запускаем симулятор с внешним портом (проверяем что в строке подключения указан порт 14551)
...
8. Запуск симуляции с jmavsim (не движется дрон):
	...
	- cd /home/dima/ardupilot							# Переходим в папку репозитория ardupilot (новый терминал)
											# Запускаем симулятор с внешними портами (проверяем что в указан порт 14551)
	- sim_vehicle.py -v ArduCopter -f quad --console --map
		/ --out=udp:127.0.0.1:14551
		/ --out=udp:0.0.0.0:14560						# (порт jmavsim)
	- python <filename>.py								# Запускаем в другом терминале VS скрипт с кодом
9. Запуск симулятора jmavsim под PX4:
	- cd ~/PX4-Autopilot 								# Переходим в папку репозитория px4
	- make px4_sitl jmavsim								# Запускаем симуляцию jmavsim
10. Запуск симулятора gazebo под PX4:
	- cd ~/PX4-Autopilot 								# Переходим в папку репозитория px4
	- make px4_sitl gz_x500								# Запускаем симуляцию gazeebo (или же gz_iris)
11. Запуск mavproxy: - mavproxy.py --master=/dev/ttyACM0 --baud=57600 --console
